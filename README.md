...
* Изменено имя GHost image на более короткое - DOS и длинные имена это FAIL

= 0.03 =  
* Временные файлы теперь располагаются на RAM Drive размером 100 MB  

= 0.05 =  
* CRC32 заменен на MD5 - выглядит более надежным
* Изменен размен RAM Drive 100 MB -> 50 MB - не все версии касс смогли потянуть 100 MB 

= 0.10 =  
* Добавлен номер версии в PARAMS.BAT 

= 0.11 =  
* Добавлена отправка номера версии на syslog сервер

= 0.12 =  
* IP адреса теперь статические, отключено получение по DHCP - много раз вызывалась команда DHCP
* tcptsr теперь грузится в обычную память - не стабильно ведет себя в верхней памяти
* Исправлены названия файлов обновлений в DBUPDATE.BAT - ошибки при copy-paste
* Изменен формат сообщения для SYSLOG сервера